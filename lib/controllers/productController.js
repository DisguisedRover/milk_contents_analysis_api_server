...
const productSchema = require('../validators/product/productValidator');

exports.getProduct = async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM product_master WHERE deleted_at IS NULL');
    res.json({
      success: true,
...
exports.deleteProduct = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      'UPDATE product_master SET deleted_at = NOW(), deleted_by = ? WHERE product_id = ?',
