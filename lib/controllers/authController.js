...
exports.loginCustomer = async (req, res) => {
  try {
    ...
    // Find customer user by email or username
    const [users] = await db.query(
      `SELECT u.*, c.customer_id, c.customer_code 
       FROM users u
       JOIN customers c ON u.id = c.user_id
       WHERE (u.email = ? OR u.username = ?) AND u.user_type = 'customer'`,
      [identifier, identifier]
    );
    ...
    if (!user.password_hash) {
      return res.status(401).json({ error: 'This account does not have login access' });
    }
    ...
    const token = jwt.sign(
      { id: user.id, user_type: 'customer', customer_id: user.customer_id }, 
      config.jwt.secret, 
      { expiresIn: config.jwt.expiresIn }
    );
